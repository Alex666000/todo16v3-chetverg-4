...
- создаем на апи отдельный объект по работе с авторизацией, используем патерн object-params - объединяем много параметров
в обхект и передаем объект целиком
для этого создаем тип сначала LoginParamsType - далее принимаем объект такого типа - отправляем весь объект без скобок
что к нам пришел data на сервер - и по хорошему надо authAPI  проверить например сэмитировать в сторибуке запрос...
...
- Открываем и видим что UX нет: но мы с кукой не работаем напрямую - нашему приложению надо узнать что мы залогинелись - сделать редирект
...
что-то -- в стейт галочку(переключалку) которая скажет что мы теперь залогинены) --- значит в какой-то редюсер надо добавить эту галочку
...
- на странице Логин прочитаем это значение залогинены ли? И если залогинены делаем редирект не на форму логина -- а на главную страницу,
- после перезагрузке стр. надо понимать -- мы залогинены или нет. Кука то существует даже после перезагрузки - но система не знает
о том что я залогинен - надо проверить залогинены или нет? Наличие куки не говорит о том что мы залогинены - залогиненость проверяется
ПРАВИЛЬНОСТЬЮ КУКИ - она должна быть (как паспорт) или токен с пправильными данными...
при загрузке нашего Арр --- надо сделать какой-нибудь запрос на сервак (если токен лежит в LS мы его цепляем к запросу...)
при каждом старте приложения (загрузке или перезагрузке) и говорим: "Кто я такой? Залогинен я или нет?
"Сервер вернет ответ кто я такой --- в документации есть такой эндпоинт
...
меняем руками -- зависимость UI от стейта редюсера (тестируем так..) -- теперь в нужный момент надо заменить на true ---- когда мы проинициализируемся
- пишем в редюсере логику изменения -- пишем санку которая будет заниматься инициализацией
она должна просто отправить запрос на сервер me и спросить залогинены мы или нет? Идем в АПИ и определим эту логику get запрос
...
- делаем предохранитель чтобы не могли попасть на Тулдулист если не залогинены редиректим на логин
 if (!isLoggedIn){
        <Navigate to={'login'}/>
    }
...
--------------------------------- ЛОГИН ФЛОУ КРАТКО:
видим крутилку изначально - инициализируем Арр(санку отправляем) в эффекте - когда Арр проинициализируется мы попытаемся попасть
на главную струницу TodolistsList со списком тудулистов но из isLoggedIn === false (мы не залогинены) поэтому редирекнет на Логин форму -
и теперь когда залогинемся и попадаем обратно к тудулистам -- и можем тут быть - потом когда обновим страницу так как кука у нас есть  и auth/me скажет
что мы залогинены isLoggedIn === true - то мы окажемся на списке тудулистов а если попытаемся в URL ввести .../login - нас выкинет обратно на тудулисты
т к мы залогинены

-------------------------------- ВЫЛОГИНЕВАНИЕ:
- сделаем кнопку logout в АРР.tsx
-  т. к колбек передаем logoutHandler - то делаем для крутой кнопки MUI - Button useCallback
